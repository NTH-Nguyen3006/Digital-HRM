...
import com.company.hrm.module.payroll.entity.*;
import com.company.hrm.module.payroll.repository.*;
...
public class PayrollService {
    ...
    public PageResponse<SalaryComponentResponse> listComponents(String keyword, RecordStatus status, int page, int size) {
        Specification<PaySalaryComponent> specification = (root, query, builder) -> builder.isFalse(root.get("deleted"));
        ...
        Page<PaySalaryComponent> result = salaryComponentRepository.findAll(specification,
                PageRequest.of(page, size, Sort.by("displayOrder").ascending().and(Sort.by("componentCode").ascending())));
        return toPageResponse(result, result.getContent().stream().map(this::toSalaryComponentResponse).toList(), page, size);
    }
    ...
    public SalaryComponentResponse upsertComponent(Long salaryComponentId, SalaryComponentUpsertRequest request) {
        ...
        if (salaryComponentId == null) {
            if (salaryComponentRepository.existsByComponentCodeIgnoreCaseAndDeletedFalse(request.componentCode().trim())) {
                throw new BusinessException("PAYROLL_COMPONENT_CODE_EXISTS", "Mã thành phần lương đã tồn tại.", HttpStatus.CONFLICT);
            }
            entity = new PaySalaryComponent();
        } else {
            entity = getSalaryComponent(salaryComponentId);
            oldSnapshot = toSalaryComponentResponse(entity);
            if (salaryComponentRepository.existsByComponentCodeIgnoreCaseAndDeletedFalseAndSalaryComponentIdNot(request.componentCode().trim(), salaryComponentId)) {
                throw new BusinessException("PAYROLL_COMPONENT_CODE_EXISTS", "Mã thành phần lương đã tồn tại.", HttpStatus.CONFLICT);
            }
        }
        ...
        auditLogService.logSuccess(salaryComponentId == null ? "CREATE" : "UPDATE", "PAYROLL_COMPONENT", "pay_salary_component",
                entity.getSalaryComponentId().toString(), oldSnapshot, response, salaryComponentId == null ? "Tạo thành phần lương." : "Cập nhật thành phần lương.");
        return response;
    }
    ...
    public FormulaVersionResponse upsertFormulaVersion(Long formulaVersionId, FormulaVersionUpsertRequest request) {
        ...
        if (formulaVersionId == null) {
            if (formulaVersionRepository.existsByFormulaCodeIgnoreCaseAndDeletedFalse(request.formulaCode().trim())) {
                throw new BusinessException("PAYROLL_FORMULA_CODE_EXISTS", "Mã cấu hình công thức lương đã tồn tại.", HttpStatus.CONFLICT);
            }
            entity = new PayFormulaVersion();
        } else {
            entity = getFormulaVersionEntity(formulaVersionId);
            oldSnapshot = toFormulaVersionResponse(entity);
            if (formulaVersionRepository.existsByFormulaCodeIgnoreCaseAndDeletedFalseAndFormulaVersionIdNot(request.formulaCode().trim(), formulaVersionId)) {
                throw new BusinessException("PAYROLL_FORMULA_CODE_EXISTS", "Mã cấu hình công thức lương đã tồn tại.", HttpStatus.CONFLICT);
            }
        }
        ...
        List<PayFormulaTaxBracket> oldBrackets = formulaTaxBracketRepository.findAllByFormulaVersionFormulaVersionIdAndDeletedFalseOrderBySequenceNoAsc(entity.getFormulaVersionId());
        ...
        auditLogService.logSuccess(formulaVersionId == null ? "CREATE" : "UPDATE", "PAYROLL_FORMULA", "pay_formula_version",
                entity.getFormulaVersionId().toString(), oldSnapshot, response, formulaVersionId == null ? "Tạo cấu hình công thức lương." : "Cập nhật cấu hình công thức lương.");
        return response;
    }

    @Transactional(readOnly = true)
    public PageResponse<EmployeeCompensationListItemResponse> listCompensations(String keyword, Long orgUnitId, int page, int size) {
        Specification<PayEmployeeCompensation> specification = (root, query, builder) -> builder.isFalse(root.get("deleted"));
        if (keyword != null && !keyword.isBlank()) {
            String like = "%" + keyword.trim().toLowerCase(Locale.ROOT) + "%";
            specification = specification.and((root, query, builder) -> builder.or(
        ...
        Page<PayEmployeeCompensation> result = compensationRepository.findAll(specification,
                PageRequest.of(page, size, Sort.by("effectiveFrom").descending().and(Sort.by("employeeCompensationId").descending())));
        List<EmployeeCompensationListItemResponse> items = result.getContent().stream().map(this::toCompensationListItem).toList();
        return toPageResponse(result, items, page, size);
    }

    @Transactional(readOnly = true)
    public EmployeeCompensationResponse getEmployeeCompensation(Long employeeId) {
        PayEmployeeCompensation compensation = compensationRepository.findAllByEmployeeEmployeeIdAndDeletedFalseOrderByEffectiveFromDescEmployeeCompensationIdDesc(employeeId)
        ...
        if (compensation == null) {
            CtLaborContract activeContract = findActiveContract(employeeId, LocalDate.now())
                    .orElseThrow(() -> new NotFoundException("PAYROLL_COMPENSATION_NOT_FOUND", "Nhân viên chưa có cấu hình lương và cũng không có hợp đồng hiệu lực."));
    ...
    @Transactional
    public EmployeeCompensationResponse upsertEmployeeCompensation(Long employeeId, EmployeeCompensationUpsertRequest request) {
        if (request.effectiveTo() != null && request.effectiveTo().isBefore(request.effectiveFrom())) {
            throw new BusinessException("PAYROLL_COMPENSATION_DATE_INVALID", "effectiveTo không được nhỏ hơn effectiveFrom.", HttpStatus.BAD_REQUEST);
        }
        ...
        if (compensationRepository.existsOverlap(employeeId, null, request.effectiveFrom(), effectiveTo)) {
            List<PayEmployeeCompensation> current = compensationRepository.findEffectiveByEmployeeAndDate(employeeId, request.effectiveFrom());
            if (!current.isEmpty()) {
    ...
    public PersonalTaxProfileResponse getPersonalTaxProfile(Long employeeId) {
        HrEmployee employee = getEmployee(employeeId);
        PayPersonalTaxProfile profile = personalTaxProfileRepository.findByEmployeeEmployeeIdAndDeletedFalse(employeeId).orElse(null);
        if (profile == null) {
    ...
    @Transactional
    public PersonalTaxProfileResponse upsertPersonalTaxProfile(Long employeeId, PersonalTaxProfileUpsertRequest request) {
        HrEmployee employee = getEmployee(employeeId);
        PayPersonalTaxProfile profile = personalTaxProfileRepository.findByEmployeeEmployeeIdAndDeletedFalse(employeeId).orElseGet(PayPersonalTaxProfile::new);
    ...
    public TaxDependentResponse addDependent(Long employeeId, TaxDependentUpsertRequest request) {
        ...
        entity.setDeductionEndMonth(request.deductionEndMonth() == null ? null : request.deductionEndMonth().withDayOfMonth(1));
        entity.setStatus(RecordStatus.ACTIVE);
        entity.setNote(trimToNull(request.note()));
        entity = taxDependentRepository.save(entity);
        TaxDependentResponse response = toTaxDependentResponse(entity);
        auditLogService.logSuccess("CREATE", "PAYROLL_TAX_DEPENDENT", "pay_tax_dependent", entity.getTaxDependentId().toString(), null, response, "Thêm người phụ thuộc.");
        return response;
    }
    ...
    public TaxDependentResponse updateDependent(Long taxDependentId, TaxDependentUpsertRequest request) {
        ...
        entity = taxDependentRepository.save(entity);
        TaxDependentResponse response = toTaxDependentResponse(entity);
        auditLogService.logSuccess("UPDATE", "PAYROLL_TAX_DEPENDENT", "pay_tax_dependent", entity.getTaxDependentId().toString(), oldSnapshot, response, "Cập nhật người phụ thuộc.");
        return response;
    }
    ...
    public TaxDependentResponse deactivateDependent(Long taxDependentId, DeactivateTaxDependentRequest request) {
        ...
        if (request.deductionEndMonth().isBefore(entity.getDeductionStartMonth())) {
            throw new BusinessException("PAYROLL_DEPENDENT_END_MONTH_INVALID", "deductionEndMonth không được nhỏ hơn deductionStartMonth.", HttpStatus.BAD_REQUEST);
        }
        ...
        entity = taxDependentRepository.save(entity);
        TaxDependentResponse response = toTaxDependentResponse(entity);
        auditLogService.logSuccess("DEACTIVATE", "PAYROLL_TAX_DEPENDENT", "pay_tax_dependent", entity.getTaxDependentId().toString(), oldSnapshot, response, "Ngừng hiệu lực giảm trừ người phụ thuộc.");
        return response;
    }
    ...
    public PayrollPeriodResponse createPayrollPeriod(PayrollPeriodCreateRequest request) {
        ...
        AttAttendancePeriod attendancePeriod = attendancePeriodRepository.findByPeriodYearAndPeriodMonthAndDeletedFalse(request.periodYear(), request.periodMonth())
                .orElseThrow(() -> new NotFoundException("PAYROLL_ATTENDANCE_PERIOD_NOT_FOUND", "Không tìm thấy kỳ công tương ứng để tạo kỳ lương."));
        if (attendancePeriod.getPeriodStatus() != AttendancePeriodStatus.CLOSED) {
            throw new BusinessException("PAYROLL_ATTENDANCE_PERIOD_NOT_CLOSED", "Chỉ được tạo kỳ lương từ kỳ công đã chốt.", HttpStatus.CONFLICT);
        }
        PayFormulaVersion formulaVersion = resolveFormulaVersion(ym.atEndOfMonth());
        PayPayrollPeriod period = payrollPeriodRepository.findByPeriodYearAndPeriodMonthAndDeletedFalse(request.periodYear(), request.periodMonth())
        ...
        auditLogService.logSuccess("CREATE", "PAYROLL_PERIOD", "pay_payroll_period", period.getPayrollPeriodId().toString(), null, response, "Tạo kỳ lương.");
        return response;
    }
    ...
    public PayrollPeriodResponse generatePayrollDraft(Long payrollPeriodId, GeneratePayrollDraftRequest request) {
        ...
        if (period.getPeriodStatus() == PayrollPeriodStatus.APPROVED || period.getPeriodStatus() == PayrollPeriodStatus.PUBLISHED) {
            throw new BusinessException("PAYROLL_PERIOD_GENERATE_STATUS_INVALID", "Không thể tạo nháp lại cho kỳ lương đã duyệt hoặc đã phát hành.", HttpStatus.CONFLICT);
        }
        List<PayPayrollItem> existingItems = payrollItemRepository.findAllByPayrollPeriodPayrollPeriodIdAndDeletedFalseOrderByEmployeeEmployeeCodeAscPayrollItemIdAsc(payrollPeriodId);
        if (!existingItems.isEmpty() && !request.regenerate()) {
            throw new BusinessException("PAYROLL_DRAFT_ALREADY_EXISTS", "Kỳ lương đã có bảng lương nháp. Dùng regenerate=true nếu muốn tính lại.", HttpStatus.CONFLICT);
        }
        ...
        long managerConfirmedCount = payrollItemRepository.countByPayrollPeriodPayrollPeriodIdAndItemStatusAndDeletedFalse(payrollPeriodId, PayrollItemStatus.MANAGER_CONFIRMED);
        ...
        PayrollPeriodResponse response = toPayrollPeriodResponse(period);
        auditLogService.logSuccess("GENERATE_DRAFT", "PAYROLL_PERIOD", "pay_payroll_period", period.getPayrollPeriodId().toString(), null, response, "Tạo bảng lương nháp.");
        return response;
    }

    @Transactional(readOnly = true)
    public PageResponse<PayrollItemResponse> listPayrollItemsForHr(Long payrollPeriodId, String keyword, Long orgUnitId, Long employeeId, int page, int size) {
        PayPayrollPeriod period = getPayrollPeriod(payrollPeriodId);
        ...
        Page<PayPayrollItem> result = payrollItemRepository.findAll(specification,
                PageRequest.of(page, size, Sort.by("employee.employeeCode").ascending().and(Sort.by("payrollItemId").ascending())));
    ...
    public PayrollItemResponse adjustPayrollItem(Long payrollItemId, AdjustPayrollItemRequest request) {
        ...
        if (item.getPayrollPeriod().getPeriodStatus() == PayrollPeriodStatus.APPROVED || item.getPayrollPeriod().getPeriodStatus() == PayrollPeriodStatus.PUBLISHED) {
            throw new BusinessException("PAYROLL_ITEM_ADJUST_STATUS_INVALID", "Không thể điều chỉnh dòng lương khi kỳ lương đã duyệt hoặc đã phát hành.", HttpStatus.CONFLICT);
        }
        PayrollItemResponse oldSnapshot = toPayrollItemResponse(item);

        List<PayPayrollItemLine> lines = payrollItemLineRepository.findAllByPayrollItemPayrollItemIdAndDeletedFalseOrderByDisplayOrderAscPayrollItemLineIdAsc(item.getPayrollItemId());
        ...
        auditLogService.logSuccess("ADJUST", "PAYROLL_ITEM", "pay_payroll_item", item.getPayrollItemId().toString(), oldSnapshot, response, "Điều chỉnh bảng lương nhân sự.");
        return response;
    }
    ...
    public List<PayrollItemResponse> listPayrollItemsForManager(Long payrollPeriodId) {
        ...
        String orgPathPrefix = accessScopeService.getManagerOrgPathPrefix()
                .orElseThrow(() -> new ForbiddenException("PAYROLL_MANAGER_SCOPE_REQUIRED", "Bạn không thuộc phạm vi quản lý để xem bảng lương team."));
    ...
    @Transactional
    public PayrollItemResponse confirmPayrollItemByManager(Long payrollItemId, ManagerConfirmPayrollItemRequest request) {
        PayPayrollItem item = getPayrollItemEntity(payrollItemId);
        accessScopeService.assertManagerCanAccessEmployee(item.getEmployee());
        if (!item.isManagerConfirmationRequired()) {
            throw new BusinessException("PAYROLL_MANAGER_CONFIRM_NOT_REQUIRED", "Dòng lương này không yêu cầu quản lý xác nhận.", HttpStatus.CONFLICT);
        }
        if (item.getPayrollPeriod().getPeriodStatus() == PayrollPeriodStatus.APPROVED || item.getPayrollPeriod().getPeriodStatus() == PayrollPeriodStatus.PUBLISHED) {
            throw new BusinessException("PAYROLL_MANAGER_CONFIRM_STATUS_INVALID", "Không thể xác nhận khi kỳ lương đã duyệt hoặc đã phát hành.", HttpStatus.CONFLICT);
        }
        PayrollItemResponse oldSnapshot = toPayrollItemResponse(item);
        ...
        period.setManagerConfirmedCount((int) payrollItemRepository.countByPayrollPeriodPayrollPeriodIdAndItemStatusAndDeletedFalse(period.getPayrollPeriodId(), PayrollItemStatus.MANAGER_CONFIRMED));
        period.setPeriodStatus(PayrollPeriodStatus.TEAM_REVIEW);
        payrollPeriodRepository.save(period);

        PayrollItemResponse response = toPayrollItemResponse(item);
        auditLogService.logSuccess("MANAGER_CONFIRM", "PAYROLL_ITEM", "pay_payroll_item", item.getPayrollItemId().toString(), oldSnapshot, response, "Quản lý xác nhận bảng lương team.");
        return response;
    }
    ...
    public PayrollPeriodResponse approvePayrollPeriod(Long payrollPeriodId, ApprovePayrollPeriodRequest request) {
        PayPayrollPeriod period = getPayrollPeriod(payrollPeriodId);
        if (period.getPeriodStatus() == PayrollPeriodStatus.APPROVED || period.getPeriodStatus() == PayrollPeriodStatus.PUBLISHED) {
            throw new BusinessException("PAYROLL_PERIOD_APPROVE_STATUS_INVALID", "Kỳ lương không còn ở trạng thái có thể duyệt.", HttpStatus.CONFLICT);
        }
        List<PayPayrollItem> items = payrollItemRepository.findAllByPayrollPeriodPayrollPeriodIdAndDeletedFalseOrderByEmployeeEmployeeCodeAscPayrollItemIdAsc(period.getPayrollPeriodId());
        if (items.isEmpty()) {
            throw new BusinessException("PAYROLL_PERIOD_EMPTY", "Kỳ lương chưa có dòng lương để duyệt.", HttpStatus.CONFLICT);
        }
        ...
        if (missingManagerConfirm > 0) {
            throw new BusinessException("PAYROLL_MANAGER_CONFIRM_PENDING", "Vẫn còn dòng lương của team chưa được quản lý xác nhận.", HttpStatus.CONFLICT);
        }
        ...
        PayrollPeriodResponse response = toPayrollPeriodResponse(period);
        auditLogService.logSuccess("APPROVE", "PAYROLL_PERIOD", "pay_payroll_period", period.getPayrollPeriodId().toString(), null, response, "HR phê duyệt bảng lương.");
        return response;
    }
    ...
    public PayrollPeriodResponse publishPayrollPeriod(Long payrollPeriodId, PublishPayrollPeriodRequest request) {
        ...
        if (period.getPeriodStatus() != PayrollPeriodStatus.APPROVED) {
            throw new BusinessException("PAYROLL_PERIOD_PUBLISH_STATUS_INVALID", "Chỉ được phát hành phiếu lương khi kỳ lương đã được duyệt.", HttpStatus.CONFLICT);
        }
        List<PayPayrollItem> items = payrollItemRepository.findAllByPayrollPeriodPayrollPeriodIdAndDeletedFalseOrderByEmployeeEmployeeCodeAscPayrollItemIdAsc(period.getPayrollPeriodId());
        LocalDateTime now = LocalDateTime.now();
        SecUserAccount currentUser = getCurrentUserOrNull();
        ...
        PayrollPeriodResponse response = toPayrollPeriodResponse(period);
        auditLogService.logSuccess("PUBLISH", "PAYROLL_PERIOD", "pay_payroll_period", period.getPayrollPeriodId().toString(), null, response, "Phát hành phiếu lương.");
        return response;
    }
    ...
    public PayrollItemResponse getMyPayslip(Long payrollPeriodId) {
        HrEmployee currentEmployee = accessScopeService.getCurrentEmployeeRequired();
        PayPayrollItem item = payrollItemRepository.findByPayrollPeriodPayrollPeriodIdAndEmployeeEmployeeIdAndDeletedFalse(payrollPeriodId, currentEmployee.getEmployeeId())
                .orElseThrow(() -> new NotFoundException("PAYROLL_PAYSLIP_NOT_FOUND", "Không tìm thấy phiếu lương cá nhân."));
    ...
    public String exportBankTransferCsv(Long payrollPeriodId) {
        PayPayrollPeriod period = getPayrollPeriod(payrollPeriodId);
        if (period.getPeriodStatus() != PayrollPeriodStatus.APPROVED && period.getPeriodStatus() != PayrollPeriodStatus.PUBLISHED) {
            throw new BusinessException("PAYROLL_BANK_EXPORT_STATUS_INVALID", "Chỉ được xuất file chi lương khi kỳ lương đã được duyệt.", HttpStatus.CONFLICT);
        }
        StringBuilder builder = new StringBuilder();
        builder.append("employeeCode,employeeName,bankAccountName,bankAccountNo,bankName,netPay,periodCode\n");
        List<PayPayrollItem> items = payrollItemRepository.findAllByPayrollPeriodPayrollPeriodIdAndDeletedFalseOrderByEmployeeEmployeeCodeAscPayrollItemIdAsc(payrollPeriodId);
        for (PayPayrollItem item : items) {
            PayEmployeeCompensation compensation = resolveCompensation(item.getEmployee().getEmployeeId(), period.getPeriodEndDate(), findActiveContract(item.getEmployee().getEmployeeId(), period.getPeriodEndDate()).orElse(null));
    ...
    public String exportPitReportCsv(Long payrollPeriodId) {
        PayPayrollPeriod period = getPayrollPeriod(payrollPeriodId);
        if (period.getPeriodStatus() != PayrollPeriodStatus.APPROVED && period.getPeriodStatus() != PayrollPeriodStatus.PUBLISHED) {
            throw new BusinessException("PAYROLL_TAX_EXPORT_STATUS_INVALID", "Chỉ được xuất báo cáo thuế khi kỳ lương đã được duyệt.", HttpStatus.CONFLICT);
        }
        StringBuilder builder = new StringBuilder();
        builder.append("employeeCode,employeeName,taxCode,grossIncome,employeeInsurance,personalDeduction,dependentDeduction,taxableIncome,pitAmount,periodCode\n");
        List<PayPayrollItem> items = payrollItemRepository.findAllByPayrollPeriodPayrollPeriodIdAndDeletedFalseOrderByEmployeeEmployeeCodeAscPayrollItemIdAsc(payrollPeriodId);
        for (PayPayrollItem item : items) {
            builder.append(csv(item.getEmployee().getEmployeeCode())).append(',')
                    .append(csv(item.getEmployee().getFullName())).append(',')
    ...
    private PayrollComputationResult computePayrollForEmployee(PayPayrollPeriod period, HrEmployee employee, PayFormulaVersion formulaVersion) {
        CtLaborContract contract = findActiveContract(employee.getEmployeeId(), period.getPeriodEndDate())
                .orElseThrow(() -> new BusinessException("PAYROLL_CONTRACT_REQUIRED", "Nhân viên " + employee.getEmployeeCode() + " chưa có hợp đồng hiệu lực để tính lương.", HttpStatus.CONFLICT));
        PayEmployeeCompensation compensation = resolveCompensation(employee.getEmployeeId(), period.getPeriodEndDate(), contract);
        ...
        for (AttDailyAttendance daily : dailyList) {
            ...
            if (daily.getDailyStatus() == AttendanceDailyStatus.PRESENT || daily.getDailyStatus() == AttendanceDailyStatus.INCOMPLETE) {
                presentDayCount++;
            }
        ...
        BigDecimal baseSalaryProrated = scheduledDayCount > 0
                ? scale(baseSalaryMonthly.multiply(BigDecimal.valueOf(payableDays)).divide(BigDecimal.valueOf(scheduledDayCount), SCALE_MONEY, RoundingMode.HALF_UP))
                : baseSalaryMonthly;
        ...
        lines.add(newLine(item, LINE_BASE, "Lương cơ bản quy đổi", PayrollComponentCategory.EARNING, PayrollLineSourceType.SYSTEM, baseSalaryProrated, true, 10, null));

        BigDecimal hourlyRate = BigDecimal.ZERO;
        if (scheduledDayCount > 0 && formulaVersion.getStandardWorkHoursPerDay() != null && formulaVersion.getStandardWorkHoursPerDay().compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal denominator = BigDecimal.valueOf(scheduledDayCount).multiply(formulaVersion.getStandardWorkHoursPerDay());
            hourlyRate = baseSalaryMonthly.divide(denominator, 8, RoundingMode.HALF_UP);
        }
        BigDecimal approvedHours = BigDecimal.valueOf(approvedOtMinutes).divide(BigDecimal.valueOf(60), 8, RoundingMode.HALF_UP);
        BigDecimal otAmount = scale(hourlyRate.multiply(approvedHours).multiply(zeroIfNull(formulaVersion.getOvertimeMultiplierWeekday())));
        if (otAmount.compareTo(BigDecimal.ZERO) > 0) {
            lines.add(newLine(item, LINE_OT, "Tiền OT được duyệt", PayrollComponentCategory.EARNING, PayrollLineSourceType.SYSTEM, otAmount, true, 20, null));
        }

        List<PayEmployeeCompensationItem> configuredItems = compensation.getEmployeeCompensationId() == null
                ? List.of()
                : compensationItemRepository.findAllByEmployeeCompensationEmployeeCompensationIdAndDeletedFalseOrderBySalaryComponentDisplayOrderAscEmployeeCompensationItemIdAsc(compensation.getEmployeeCompensationId());
        ...
        BigDecimal insuranceBase = compensation.getInsuranceSalaryBase() != null ? compensation.getInsuranceSalaryBase() : baseSalaryMonthly;
        if (formulaVersion.getInsuranceSalaryCapAmount() != null && formulaVersion.getInsuranceSalaryCapAmount().compareTo(BigDecimal.ZERO) > 0) {
            insuranceBase = insuranceBase.min(formulaVersion.getInsuranceSalaryCapAmount());
        }
        BigDecimal unemploymentBase = compensation.getInsuranceSalaryBase() != null ? compensation.getInsuranceSalaryBase() : baseSalaryMonthly;
        if (formulaVersion.getUnemploymentSalaryCapAmount() != null && formulaVersion.getUnemploymentSalaryCapAmount().compareTo(BigDecimal.ZERO) > 0) {
            unemploymentBase = unemploymentBase.min(formulaVersion.getUnemploymentSalaryCapAmount());
        }

        BigDecimal socialAmount = scale(insuranceBase.multiply(zeroIfNull(formulaVersion.getSocialInsuranceEmployeeRate())).divide(ONE_HUNDRED, 8, RoundingMode.HALF_UP));
        BigDecimal healthAmount = scale(insuranceBase.multiply(zeroIfNull(formulaVersion.getHealthInsuranceEmployeeRate())).divide(ONE_HUNDRED, 8, RoundingMode.HALF_UP));
        BigDecimal unempAmount = scale(unemploymentBase.multiply(zeroIfNull(formulaVersion.getUnemploymentInsuranceEmployeeRate())).divide(ONE_HUNDRED, 8, RoundingMode.HALF_UP));
        if (socialAmount.compareTo(BigDecimal.ZERO) > 0) {
            lines.add(newLine(item, LINE_SOCIAL, "BHXH người lao động", PayrollComponentCategory.DEDUCTION, PayrollLineSourceType.SYSTEM, socialAmount, false, 900, null));
        }
        if (healthAmount.compareTo(BigDecimal.ZERO) > 0) {
            lines.add(newLine(item, LINE_HEALTH, "BHYT người lao động", PayrollComponentCategory.DEDUCTION, PayrollLineSourceType.SYSTEM, healthAmount, false, 901, null));
        }
        if (unempAmount.compareTo(BigDecimal.ZERO) > 0) {
            lines.add(newLine(item, LINE_UNEMP, "BHTN người lao động", PayrollComponentCategory.DEDUCTION, PayrollLineSourceType.SYSTEM, unempAmount, false, 902, null));
        }
        payrollItemLineRepository.saveAll(lines);

        PersonalTaxContext taxContext = resolvePersonalTaxContext(employee.getEmployeeId(), period.getPeriodEndDate(), formulaVersion);
        ...
        return new PayrollComputationResult(item, payrollItemLineRepository.findAllByPayrollItemPayrollItemIdAndDeletedFalseOrderByDisplayOrderAscPayrollItemLineIdAsc(item.getPayrollItemId()));
    }
    ...
    private void recalculateItemTotalsFromLines(PayPayrollItem item) {
        ...
        for (PayPayrollItemLine line : lines) {
            if (line.getComponentCategory() == PayrollComponentCategory.EARNING) {
                ...
                taxableAddition = taxableAddition.add(line.isTaxable() ? zeroIfNull(line.getLineAmount()) : BigDecimal.ZERO);
        ...
        PayPayrollItemLine currentPitLine = lines.stream().filter(line -> LINE_PIT.equals(line.getComponentCode()) && !line.isDeleted()).findFirst().orElse(null);
        if (currentPitLine == null && pitAmount.compareTo(BigDecimal.ZERO) > 0) {
            currentPitLine = newLine(item, LINE_PIT, "Thuế TNCN tạm khấu trừ", PayrollComponentCategory.DEDUCTION, PayrollLineSourceType.SYSTEM, pitAmount, false, 990, null);
    ...
    private PersonalTaxContext resolvePersonalTaxContext(Long employeeId, LocalDate periodEndDate, PayFormulaVersion formulaVersion) {
        PayPersonalTaxProfile profile = personalTaxProfileRepository.findByEmployeeEmployeeIdAndDeletedFalse(employeeId).orElse(null);
        ...
        if (residentTaxpayer && familyDeductionEnabled) {
            List<PayTaxDependent> dependents = taxDependentRepository.findAllByEmployeeEmployeeIdAndStatusAndDeletedFalseOrderByDeductionStartMonthAscTaxDependentIdAsc(employeeId, RecordStatus.ACTIVE);
            ...
            activeDependentCount = dependents.stream()
                    ...
                    .filter(dep -> dep.getDeductionEndMonth() == null || !dep.getDeductionEndMonth().isBefore(monthStart))
                    .count();
        }
        BigDecimal dependentDeduction = residentTaxpayer && familyDeductionEnabled
                ? scale(formulaVersion.getDependentDeductionMonthly().multiply(BigDecimal.valueOf(activeDependentCount)))
                : BigDecimal.ZERO;
    ...
    private BigDecimal computePitAmount(BigDecimal taxableIncome, PayFormulaVersion formulaVersion) {
        ...
        List<PayFormulaTaxBracket> brackets = formulaTaxBracketRepository.findAllByFormulaVersionFormulaVersionIdAndDeletedFalseOrderBySequenceNoAsc(formulaVersion.getFormulaVersionId());
        for (PayFormulaTaxBracket bracket : brackets) {
            ...
            boolean upperBoundMatched = bracket.getIncomeTo() == null || taxableIncome.compareTo(bracket.getIncomeTo()) <= 0;
    ...
    private boolean isPaidLeave(Long employeeId, LocalDate date) {
        return leaveRequestRepository.existsFinalizedLeaveOnDateAndPaid(employeeId, date, LeaveRequestStatus.FINALIZED, true);
    }
    ...
    private List<HrEmployee> findEligibleEmployees(LocalDate periodEndDate) {
        return employeeRepository.findAll((root, query, builder) -> builder.and(
                        ...
                        builder.lessThanOrEqualTo(root.get("hireDate"), periodEndDate)
                ), Sort.by("employeeCode").ascending());
    }
    ...
    private void validateEmployeesHaveContracts(List<HrEmployee> employees, LocalDate date) {
        List<String> missing = employees.stream()
                ...
                .toList();
        if (!missing.isEmpty()) {
            throw new BusinessException("PAYROLL_ACTIVE_CONTRACT_REQUIRED", "Có nhân viên chưa có hợp đồng hiệu lực để tính lương.", HttpStatus.CONFLICT, missing);
        }
    }
    ...
    private PayEmployeeCompensation resolveCompensation(Long employeeId, LocalDate date, CtLaborContract contract) {
        PayEmployeeCompensation compensation = compensationRepository.findEffectiveByEmployeeAndDate(employeeId, date).stream().findFirst().orElse(null);
        if (compensation != null) {
            return compensation;
        }
        if (contract == null) {
    ...
    private void refreshPeriodAggregates(PayPayrollPeriod period) {
        ...
        period.setManagerConfirmedCount((int) items.stream().filter(item -> item.getItemStatus() == PayrollItemStatus.MANAGER_CONFIRMED).count());
        period.setTotalGrossAmount(scale(items.stream().map(PayPayrollItem::getGrossIncome).filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add)));
        period.setTotalNetAmount(scale(items.stream().map(PayPayrollItem::getNetPay).filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add)));
        payrollPeriodRepository.save(period);
    }
    ...
    private BigDecimal computeConfiguredAmount(BigDecimal baseSalary, PayEmployeeCompensationItem item) {
        ...
        if (amountType == PayrollAmountType.PERCENT_BASE) {
            return scale(baseSalary.multiply(zeroIfNull(item.getPercentageValue())).divide(ONE_HUNDRED, 8, RoundingMode.HALF_UP));
        }
        return BigDecimal.ZERO.setScale(SCALE_MONEY, RoundingMode.HALF_UP);
    }
    ...
    private void validateFormulaRequest(FormulaVersionUpsertRequest request) {
        if (request.effectiveTo() != null && request.effectiveTo().isBefore(request.effectiveFrom())) {
            throw new BusinessException("PAYROLL_FORMULA_DATE_INVALID", "effectiveTo không được nhỏ hơn effectiveFrom.", HttpStatus.BAD_REQUEST);
        }
        ...
        for (FormulaTaxBracketRequest bracket : brackets) {
            if (bracket.incomeTo() != null && bracket.incomeTo().compareTo(bracket.incomeFrom()) < 0) {
                throw new BusinessException("PAYROLL_FORMULA_BRACKET_INVALID", "incomeTo không được nhỏ hơn incomeFrom.", HttpStatus.BAD_REQUEST);
            }
            if (previousTo != null && bracket.incomeFrom().compareTo(previousTo) <= 0) {
                throw new BusinessException("PAYROLL_FORMULA_BRACKET_INVALID", "Các bậc thuế đang bị chồng lấn hoặc chạm biên không hợp lệ.", HttpStatus.BAD_REQUEST);
            }
    ...
    private void validateCompensationItems(List<EmployeeCompensationItemRequest> items) {
        ...
        for (EmployeeCompensationItemRequest item : items) {
            if (!seen.add(item.salaryComponentId())) {
                throw new BusinessException("PAYROLL_COMPENSATION_ITEM_DUPLICATE", "Không được lặp lại salaryComponentId trong cùng một cấu hình lương.", HttpStatus.BAD_REQUEST);
            }
            ...
            if (component.getAmountType() == PayrollAmountType.FIXED_AMOUNT && item.amountValue() == null) {
                throw new BusinessException("PAYROLL_COMPENSATION_ITEM_AMOUNT_REQUIRED", "Component FIXED_AMOUNT bắt buộc có amountValue.", HttpStatus.BAD_REQUEST);
            }
            if (component.getAmountType() == PayrollAmountType.PERCENT_BASE && item.percentageValue() == null) {
                throw new BusinessException("PAYROLL_COMPENSATION_ITEM_PERCENT_REQUIRED", "Component PERCENT_BASE bắt buộc có percentageValue.", HttpStatus.BAD_REQUEST);
            }
        }
    }
    ...
    private void validateDependentDates(LocalDate startMonth, LocalDate endMonth) {
        if (endMonth != null && endMonth.isBefore(startMonth)) {
            throw new BusinessException("PAYROLL_DEPENDENT_DATE_INVALID", "deductionEndMonth không được nhỏ hơn deductionStartMonth.", HttpStatus.BAD_REQUEST);
        }
    }
    ...
    private PayrollComponentCategory parseCategory(String value) {
        try {
            ...
            throw new BusinessException("PAYROLL_LINE_CATEGORY_INVALID", "componentCategory không hợp lệ.", HttpStatus.BAD_REQUEST);
        }
    }
    ...
    private PaySalaryComponent getSalaryComponent(Long salaryComponentId) {
        return salaryComponentRepository.findBySalaryComponentIdAndDeletedFalse(salaryComponentId)
                .orElseThrow(() -> new NotFoundException("PAYROLL_COMPONENT_NOT_FOUND", "Không tìm thấy thành phần lương."));
    }
    ...
    private PayFormulaVersion getFormulaVersionEntity(Long formulaVersionId) {
        return formulaVersionRepository.findByFormulaVersionIdAndDeletedFalse(formulaVersionId)
                .orElseThrow(() -> new NotFoundException("PAYROLL_FORMULA_NOT_FOUND", "Không tìm thấy cấu hình công thức lương."));
    }
    ...
    private PayFormulaVersion resolveFormulaVersion(LocalDate date) {
        return formulaVersionRepository.findEffectiveVersions(date).stream().findFirst()
                .orElseThrow(() -> new NotFoundException("PAYROLL_EFFECTIVE_FORMULA_NOT_FOUND", "Không tìm thấy cấu hình công thức lương hiệu lực cho kỳ này."));
    }
    ...
    private PayTaxDependent getTaxDependent(Long taxDependentId) {
        return taxDependentRepository.findByTaxDependentIdAndDeletedFalse(taxDependentId)
                .orElseThrow(() -> new NotFoundException("PAYROLL_DEPENDENT_NOT_FOUND", "Không tìm thấy người phụ thuộc."));
    }
    ...
    private FormulaVersionResponse toFormulaVersionResponse(PayFormulaVersion entity) {
        List<FormulaTaxBracketResponse> taxBrackets = formulaTaxBracketRepository.findAllByFormulaVersionFormulaVersionIdAndDeletedFalseOrderBySequenceNoAsc(entity.getFormulaVersionId())
                .stream()
    ...
    private EmployeeCompensationResponse toEmployeeCompensationResponse(PayEmployeeCompensation entity) {
        List<EmployeeCompensationItemResponse> items = entity.getEmployeeCompensationId() == null ? List.of()
                : compensationItemRepository.findAllByEmployeeCompensationEmployeeCompensationIdAndDeletedFalseOrderBySalaryComponentDisplayOrderAscEmployeeCompensationItemIdAsc(entity.getEmployeeCompensationId())
                .stream()
    ...
    private PersonalTaxProfileResponse toPersonalTaxProfileResponse(PayPersonalTaxProfile profile) {
        List<TaxDependentResponse> dependents = taxDependentRepository.findAllByEmployeeEmployeeIdAndDeletedFalseOrderByDeductionStartMonthAscTaxDependentIdAsc(profile.getEmployee().getEmployeeId())
    ...
    private PayrollItemResponse toPayrollItemResponse(PayPayrollItem entity) {
        List<PayrollItemLineResponse> lines = payrollItemLineRepository.findAllByPayrollItemPayrollItemIdAndDeletedFalseOrderByDisplayOrderAscPayrollItemLineIdAsc(entity.getPayrollItemId())
                .stream()
