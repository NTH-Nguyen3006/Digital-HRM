...
public class AuthService {
        ...
        public AuthResult login(LoginRequest request) {
                ...
                if (activeRole.getRole().getStatus() != RecordStatus.ACTIVE) {
                        throw new UnauthorizedException("AUTH_ROLE_INACTIVE",
                                        "Role chính của tài khoản đang không hoạt động.");
                }
        ...
        public void forgotPassword(ForgotPasswordRequest request) {
                SecUserAccount user = userAccountRepository.findByEmailIgnoreCaseAndDeletedFalse(request.email())
                                .orElse(null);
                if (user == null) {
                        return;
                }
        ...
        public void resetPassword(ResetPasswordRequest request) {
                ...
                if (token.getExpiresAt().isBefore(LocalDateTime.now())) {
                        ...
                        passwordResetTokenRepository.save(token);
                        throw new BusinessException("RESET_TOKEN_EXPIRED", "Token reset đã hết hạn.",
                                        HttpStatus.BAD_REQUEST);
                }
        ...
        private void handleFailedLogin(SecUserAccount user) {
                ...
                if (failed >= appProperties.getAuth().getMaxFailedLoginAttempts()) {
                        ...
                        user.setLockedUntil(
                                        LocalDateTime.now().plusMinutes(appProperties.getAuth().getAutoLockMinutes()));
                }
