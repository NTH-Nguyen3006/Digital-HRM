...
const selectedLeaveRule = computed(() => selectedLeaveType.value?.currentRule || null)
...
const requestedUnits = computed(() => {
  if (!form.value.startDate || !form.value.endDate) return 0
  if (form.value.startDate > form.value.endDate) return 0

  if (selectedLeaveRule.value?.unitType === 'HOUR') {
    return 0
  }
...
const leaveBalanceCheckState = computed(() => {
  ...
  const requiresBalanceCheck = Boolean(selectedLeaveRule.value?.requiresBalanceCheck)
  const allowNegativeBalance = Boolean(selectedLeaveRule.value?.allowNegativeBalance)
  const availableUnits = selectedLeaveType.value.availableUnits === null
    ? null
    : Number(selectedLeaveType.value.availableUnits || 0)
  const normalizedAvailableUnits = availableUnits === null ? 0 : availableUnits
  const isInsufficient = requiresBalanceCheck && !allowNegativeBalance && requestedUnits.value > 0 && normalizedAvailableUnits < requestedUnits.value
  ...
  return {
    ...
    requestedUnits: requestedUnits.value,
    isInsufficient,
    shortageUnits: isInsufficient ? requestedUnits.value - normalizedAvailableUnits : 0,
  }
})
...
const handleSubmit = async () => {
  ...
  if (leaveBalanceCheckState.value.isInsufficient) {
    toast.warning('Loại nghỉ đã chọn không đủ số dư. Vui lòng đổi loại nghỉ hoặc điều chỉnh lại thời gian.')
    return
  }
...
<template>
  <div class="mx-auto max-w-6xl space-y-8 px-4 pb-10 md:px-6">
    <section
      class="rounded-4xl border border-slate-200/80 bg-[linear-gradient(135deg,#ffffff_0%,#f8fbff_100%)] p-6 shadow-[0_18px_60px_rgba(15,23,42,0.06)] md:p-8">
      ...
      <div class="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div v-for="item in leaveStats" :key="item.label"
      ...
      <div class="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_16px_50px_rgba(15,23,42,0.05)] md:p-8">
        <div class="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p class="mt-1 text-sm font-medium text-slate-500">Danh sách loại nghỉ đang áp dụng cho tài khoản của bạn.
            </p>
          </div>
        </div>
        ...
        <div v-else-if="balances.length" class="grid gap-4 sm:grid-cols-2">
          <article v-for="balance in balances" :key="balance.leaveBalanceId || balance.leaveTypeId"
            class="rounded-[28px] border border-slate-200/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] p-5">
            <div class="flex items-start justify-between gap-4">
              <div>
                <p class="text-[11px] font-black uppercase tracking-[0.18em] text-indigo-500">{{ balance.leaveTypeName
                  }}</p>
                <p class="mt-2 text-sm font-semibold text-slate-500">{{ balance.leaveTypeCode || 'Không có mã loại nghỉ'
                  }}</p>
              </div>
              <span
                class="rounded-full bg-slate-100 px-3 py-1 text-[11px] font-black uppercase tracking-wider text-slate-600">
            ...
            <div class="mt-5 grid gap-3 text-sm font-medium text-slate-500 sm:grid-cols-2">
              ...
              <div class="rounded-2xl bg-slate-50 p-3">
                ...
                <p class="mt-2 text-lg font-black text-slate-900">{{ formatUnits(balance.totalUnits ||
                  balance.openingUnits + balance.accruedUnits) }}</p>
              </div>
            </div>
          </article>
        </div>
        ...
        <div v-else class="rounded-[28px] border border-dashed border-slate-200 bg-slate-50 px-6 py-10 text-center">
          ...
          <p class="mt-2 text-sm font-medium text-slate-500">Cần HR cấp số dư phép hoặc seed dữ liệu loại nghỉ cho tài
            khoản này.</p>
        </div>
      </div>

      <div
        class="rounded-[30px] border border-slate-200 bg-slate-950 p-6 text-white shadow-[0_20px_60px_rgba(15,23,42,0.16)] md:p-8">
        ...
        <div class="mt-6 space-y-5">
          <div class="rounded-[24px] border border-white/10 bg-white/5 p-4">
            ...
            <p class="mt-2 text-lg font-black text-white">{{ selectedLeaveType?.leaveTypeName || 'Chưa chọn loại nghỉ'
              }}</p>
    ...
    <section
      class="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_16px_50px_rgba(15,23,42,0.05)] md:p-8">
      <div class="mb-6 flex items-center gap-3">
        <Clock3 class="h-5 w-5 text-indigo-500" />
        <div>
          <h2 class="text-2xl font-black text-slate-900">Lịch sử đơn nghỉ</h2>
          <p class="mt-1 text-sm font-medium text-slate-500">Theo dõi loại nghỉ, mã đơn, trạng thái và thời điểm gửi
            đơn.</p>
        </div>
      </div>
      ...
      <div v-else-if="leaves.length" class="space-y-4">
        <article v-for="leave in leaves" :key="leave.leaveRequestId"
          ...
          <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-3">
                ...
                <span class="rounded-full border px-3 py-1 text-[11px] font-black uppercase tracking-wider"
                  :class="statusClass(leave.requestStatus)">
            ...
            <div class="rounded-[22px] bg-slate-50 px-4 py-3 text-sm font-medium text-slate-500 xl:min-w-[220px]">
              ...
              <p class="mt-2"><span class="font-black text-slate-900">Cấp duyệt:</span> {{ leave.approvalRoleCode ||
                'Theo workflow hệ thống' }}</p>
            </div>
          </div>
        </article>
      </div>
      ...
      <div v-else class="rounded-[28px] border border-dashed border-slate-200 bg-slate-50 px-6 py-12 text-center">
        ...
        <p class="mt-2 text-sm font-medium text-slate-500">Khi bạn tạo đơn mới, toàn bộ lịch sử xử lý sẽ hiển thị ở khu
          vực này.</p>
      </div>
    </section>
  </div>
        ...
        <div class="relative z-10 w-full max-w-xl rounded-[28px] bg-white p-6 shadow-2xl md:p-7">
          ...
          <div class="mt-6 space-y-4">
            <div>
              ...
              <select v-model="form.leaveTypeId"
                ...
                <option v-for="item in leaveTypeOptions" :key="item.leaveTypeId" :value="String(item.leaveTypeId)">
                  {{
                    item.currentRule?.requiresBalanceCheck && !item.currentRule?.allowNegativeBalance
                      ? `${item.leaveTypeName} (${item.availableUnits === null ? 'Chưa có số dư' : `Còn ${formatUnits(item.availableUnits)} ngày`})`
            ...
            <div
              ...
              class="rounded-[22px] border p-4 text-sm font-medium"
              :class="leaveBalanceCheckState.isInsufficient
                ? 'border-rose-200 bg-rose-50/80 text-rose-700'
                : 'border-indigo-100 bg-indigo-50/70 text-slate-600'"
            >
              Bạn đang chọn <span class="font-black text-slate-900">{{ selectedLeaveType.leaveTypeName }}</span>.
              <template v-if="leaveBalanceCheckState.requiresBalanceCheck && !leaveBalanceCheckState.allowNegativeBalance">
                <template v-if="leaveBalanceCheckState.availableUnits !== null">
                  Hệ thống hiện ghi nhận còn
                  <span class="font-black" :class="leaveBalanceCheckState.isInsufficient ? 'text-rose-700' : 'text-indigo-600'">
                    {{ formatUnits(leaveBalanceCheckState.availableUnits) }} ngày
                  </span>
                  khả dụng.
                </template>
                <template v-else>
                  Hệ thống hiện chưa có số dư phép khả dụng cho loại nghỉ này.
                </template>
                <template v-if="leaveBalanceCheckState.requestedUnits > 0">
                  Đơn này đang cần
                  <span class="font-black" :class="leaveBalanceCheckState.isInsufficient ? 'text-rose-700' : 'text-indigo-600'">
                    {{ formatUnits(leaveBalanceCheckState.requestedUnits) }} ngày
                  </span>.
                </template>
                <template v-if="leaveBalanceCheckState.isInsufficient">
                  Vui lòng chọn loại nghỉ khác như <span class="font-black text-rose-700">Nghỉ không lương</span> hoặc rút ngắn thời gian nghỉ.
                </template>
              </template>
              <template v-else-if="selectedLeaveType.availableUnits !== null">
              ...
              <template v-else>
                Loại nghỉ này không bắt buộc kiểm tra số dư phép, bạn có thể tiếp tục gửi đơn nếu đáp ứng các rule còn lại.
              </template>
            </div>
          ...
          <div class="mt-6 flex gap-3">
            ...
            <button @click="handleSubmit" :disabled="submitting || leaveBalanceCheckState.isInsufficient"
              class="flex flex-1 items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3 font-bold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">
              <Loader2 v-if="submitting" class="h-4 w-4 animate-spin" />
              {{ leaveBalanceCheckState.isInsufficient ? 'Không đủ số dư để gửi' : 'Gửi đơn' }}
            </button>
          </div>
        </div>
      </div>
