...
<template>
  ...
  <div class="space-y-8 animate-fade-in">
    ...
    <!-- HEADER -->
    <PageHeader title="Phân quyền Hệ thống"
      subtitle="Phân tích tính năng HR và đồng bộ dữ liệu role, permission, menu từ backend ra giao diện"
      :icon="ShieldCheck" iconColor="bg-violet-600" iconShadow="shadow-violet-100" />
    ...
    <div class="relative min-h-[300px]">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        <div v-for="role in roles" :key="role.roleId" @click="openPanel(role)"
          ...
          <div class="flex justify-end mb-4">
            <span
              class="inline-flex items-center gap-1 whitespace-nowrap px-2.5 py-1 rounded-lg text-[10px] font-black leading-none"
          ...
          <div class="flex items-center justify-between pt-4 border-t border-slate-50">
            ...
            <div class="flex items-center gap-2">
              <button v-if="!role.systemRole" @click.stop="toggleRoleStatus(role)"
      ...
      <EmptyState v-if="!loading && roles.length === 0" title="Chưa có role nào"
        ...
        <div v-else class="space-y-4">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="text-xs font-bold text-slate-400">
              Hiển thị {{ permissionMatrixRange.from }} - {{ permissionMatrixRange.to }} / {{ permissionMatrixTotal }}
              quyền
            </div>

            <div class="flex flex-wrap items-center gap-2">
              <label class="text-[11px] font-black uppercase tracking-widest text-slate-400">Mỗi trang</label>
              <select v-model.number="permissionMatrixPageSize"
          ...
          <div class="overflow-x-auto">
            <table class="w-full min-w-[720px] text-sm">
              ...
              <tbody>
                <tr v-for="row in paginatedPermissionMatrix" :key="row.permissionCode"
                  class="border-b border-slate-50 last:border-b-0">
                  <td class="py-3 pr-4 align-top">
                    <span
                      class="inline-flex px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 text-[11px] font-bold uppercase tracking-wide">
                  ...
                  <td class="py-3 pr-4">
                    <Check v-if="row.roleAllowed?.ADMIN" class="w-4 h-4 text-emerald-600" />
                    <X v-else class="w-4 h-4 text-slate-300" />
                  </td>
                  <td class="py-3 pr-4">
                    <Check v-if="row.roleAllowed?.HR" class="w-4 h-4 text-emerald-600" />
                    <X v-else class="w-4 h-4 text-slate-300" />
                  </td>
                  <td class="py-3 pr-4">
                    <Check v-if="row.roleAllowed?.MANAGER" class="w-4 h-4 text-emerald-600" />
                    <X v-else class="w-4 h-4 text-slate-300" />
                  </td>
                  <td class="py-3">
                    <Check v-if="row.roleAllowed?.EMPLOYEE" class="w-4 h-4 text-emerald-600" />
                    <X v-else class="w-4 h-4 text-slate-300" />
                  </td>
                </tr>
              </tbody>
          ...
          <div v-else
            ...
            <div class="flex flex-wrap items-center justify-end gap-2">
              <button type="button"
                ...
                :disabled="isFirstPermissionPage" @click="prevPermissionPage">
                Trước
              </button>

              <template v-for="page in visiblePermissionPages" :key="`page-${page}`">
                <span v-if="page === '...'" class="px-1 text-sm font-black text-slate-300">
                  ...
                </span>
                <button v-else type="button" class="h-10 min-w-10 rounded-xl border px-3 text-sm font-black transition"
              ...
              <button type="button"
                class="rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600 transition hover:border-violet-200 hover:text-violet-700 disabled:cursor-not-allowed disabled:opacity-40"
                :disabled="isLastPermissionPage" @click="nextPermissionPage">
                Sau
              </button>
            </div>
          </div>
      ...
      <div class="rounded-[28px] bg-gradient-to-br from-slate-900 via-slate-800 to-violet-900 p-6 text-white shadow-sm">
        ...
        <div class="space-y-3 text-sm text-slate-200/90">
          <div class="rounded-2xl bg-white/10 p-4">
            ...
            dùng để xác định ai được truy cập các chức năng như quản trị người dùng, audit log, báo cáo hoặc cấu hình hệ
            thống.
          </div>
          <div class="rounded-2xl bg-white/10 p-4">
            <span class="font-black text-white">Permission</span>
            là lớp quyền chi tiết theo từng module HR như báo cáo, payroll, attendance, leave, onboarding hoặc
            governance.
          </div>
          <div class="rounded-2xl bg-white/10 p-4">
            ...
            <div class="flex-1 overflow-y-auto p-8 space-y-6">
              <div class="bg-violet-50 border border-violet-100 rounded-[24px] p-5">
                ...
                <div v-if="getEnabledFeatureGroups(selectedRole).length" class="space-y-3">
                  <div v-for="group in getEnabledFeatureGroups(selectedRole)" :key="group.moduleCode"
                    ...
                    <div class="flex flex-wrap gap-2">
                      <span v-for="item in group.items" :key="item.permissionCode"
              ...
              <div class="bg-sky-50 border border-sky-100 rounded-[24px] p-5">
                ...
                <div v-if="roleMenuConfigs.length" class="space-y-3">
                  <div v-for="menu in roleMenuConfigs" :key="menu.roleMenuConfigId"
              ...
              <div v-for="(perms, module) in groupPermissions(selectedRole.permissions)" :key="module"
                class="bg-slate-50 rounded-[24px] p-5">
                <div class="flex items-center gap-2 mb-4">
                  <span
                    class="px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider bg-white shadow-sm text-slate-700 border border-slate-100">
                    {{ module }}
                  </span>
                </div>
                <div class="grid grid-cols-2 gap-2">
                  <div v-for="perm in perms" :key="perm.permissionCode"
                    ...
                    <span class="text-xs font-bold text-slate-700 truncate">{{ perm.permissionName || perm.actionCode
                      }}</span>
                  </div>
                </div>
              </div>

              <div v-if="!selectedRole.permissions?.length" class="text-center py-8 text-sm text-slate-400 font-medium">
