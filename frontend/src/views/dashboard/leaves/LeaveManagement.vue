...
<template>
  <div class="space-y-8">
    <PageHeader title="Quản lý nghỉ phép" subtitle="HR theo dõi đơn nghỉ, quỹ phép và các trường hợp cần xử lý nhanh."
      :icon="CalendarDays">
      <template #actions>
    ...
    <SurfacePanel>
      <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div class="flex flex-col gap-3 md:flex-row">
          <div v-if="activeTab === 'requests'" class="flex flex-wrap gap-2">
            <button v-for="status in ['ALL', 'SUBMITTED', 'APPROVED', 'FINALIZED', 'REJECTED']" :key="status"
              type="button" class="rounded-full px-3 py-2 text-xs font-black uppercase tracking-[0.18em] transition-all"
          ...
          <div class="relative">
            ...
            <input v-model="searchQuery" type="text" placeholder="Tìm nhân sự hoặc loại nghỉ..."
      ...
      <div v-else-if="activeTab === 'requests'" class="mt-6 space-y-4">
        <div v-for="item in requests" :key="item.leaveRequestId"
          ...
          <div class="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div class="grid gap-4 md:grid-cols-3 xl:min-w-105">
              <div class="rounded-2xl bg-slate-50 p-4">
                <p class="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Khoảng nghỉ</p>
                <p class="mt-2 text-sm font-bold text-slate-800">{{ formatDate(item.startDate) }} - {{
          ...
          <div v-if="['SUBMITTED', 'APPROVED'].includes(item.requestStatus)"
        ...
        <EmptyState v-if="!requests.length" iconName="CalendarCheck" title="Không có đơn nghỉ phù hợp"
      ...
      <div v-else class="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        <div v-for="item in balances"
          ...
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <h4 class="mt-3 text-lg font-black text-slate-900">{{ item.employeeName || `Nhân sự #${item.employeeId}`
                }}
              </h4>
          ...
          <div class="mt-5 rounded-[24px] bg-slate-50 p-4">
            ...
            <div
              class="mt-3 flex items-center justify-between text-xs font-bold uppercase tracking-[0.16em] text-slate-400">
        ...
        <div v-if="!balances.length" class="md:col-span-2 xl:col-span-3">
          <EmptyState iconName="Wallet" title="Chưa có quỹ phép để hiển thị"
