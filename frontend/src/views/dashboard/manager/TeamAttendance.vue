...
<template>
  ...
  <div class="space-y-8 animate-fade-in">
    ...
    <!-- HEADER -->
    <PageHeader title="Duyệt Chấm công & Tăng ca" subtitle="Phê duyệt điều chỉnh giờ công và yêu cầu OT của đội nhóm"
      :icon="Clock" iconColor="bg-indigo-600" iconShadow="shadow-indigo-100" />
    ...
    <div class="bg-white border border-slate-100 rounded-4xl overflow-hidden shadow-sm">
      ...
      <div class="px-8 border-b border-slate-50 flex gap-2">
        <button v-for="tab in [
          ...
          <div v-if="activeTab === tab.key" class="absolute bottom-0 left-0 right-0 h-1 bg-indigo-500 rounded-t-full" />
        </button>
      </div>
      ...
      <div class="relative min-h-[400px]">
        ...
        <div v-if="activeTab === 'adjustment'">
          <div v-if="adjustments.length > 0" class="divide-y divide-slate-50">
            <div v-for="req in adjustments" :key="req.adjustmentRequestId"
          ...
          <EmptyState v-else-if="!loading" title="Không có yêu cầu điều chỉnh"
            description="Không có đơn điều chỉnh chấm công nào đang chờ xử lý." iconName="Clock" />
        </div>

        <!-- OVERTIME REQUESTS -->
        <div v-if="activeTab === 'overtime'">
          <div v-if="overtimes.length > 0" class="divide-y divide-slate-50">
            <div v-for="req in overtimes" :key="req.overtimeRequestId"
              class="group px-8 py-6 hover:bg-slate-50/60 transition-all flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div class="flex items-start gap-4 min-w-[250px]">
                <AvatarBox :name="req.employeeName" size="lg" shape="rounded-[20px]" shadow />
                <div>
          ...
          <EmptyState v-else-if="!loading" title="Không có yêu cầu OT"
            description="Không có yêu cầu tăng ca nào đang chờ phê duyệt." iconName="TrendingUp" />
        </div>
      </div>
    </div>
