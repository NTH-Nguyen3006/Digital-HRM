...
<template>
  ...
  <div class="space-y-8 animate-fade-in">
    ...
    <!-- HEADER -->
    <PageHeader title="Bảng lương Đội nhóm" subtitle="Xem xét và xác nhận phiếu lương cho từng thành viên"
      :icon="Banknote" iconColor="bg-emerald-600" iconShadow="shadow-emerald-100" />
    ...
    <template v-else>
      ...
      <div v-if="periods.length === 0"
        class="bg-emerald-50 border border-emerald-100 rounded-3xl p-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-sm text-emerald-700 font-medium mt-1">HR chưa mở kỳ tính lương. Khi có kỳ lương, danh sách sẽ
            hiện tại đây.</p>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div class="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          <button v-for="p in periods" :key="p.payrollPeriodId"
            @click="selectedPeriodId = p.payrollPeriodId; fetchItems()"
            class="shrink-0 px-5 py-3 rounded-2xl text-sm font-black transition-all" :class="selectedPeriodId === p.payrollPeriodId
        ...
        <div v-if="items.length"
          class="flex items-center gap-3 px-6 py-4 bg-emerald-50 border border-emerald-100 rounded-3xl">
        ...
        <!-- Payroll cards -->
        <div class="bg-white border border-slate-100 rounded-4xl overflow-hidden shadow-sm relative">
          ...
          <div v-if="items.length > 0" class="divide-y divide-slate-50">
            <div v-for="item in items" :key="item.payrollItemId"
              ...
              <div class="shrink-0 flex items-center gap-3">
                ...
                <div v-if="item.managerConfirmedAt || item.itemStatus === 'MANAGER_CONFIRMED'"
                  class="flex items-center gap-2 px-4 py-2.5 bg-emerald-50 rounded-2xl">
                  <CheckCircle2 class="w-4 h-4 text-emerald-600" />
                  <span class="text-xs font-black text-emerald-700">Đã xác nhận</span>
                </div>
                <button v-else @click="handleConfirm(item)" :disabled="confirmLoading === item.payrollItemId"
          ...
          <EmptyState v-else-if="!loading" title="Chưa có dữ liệu lương"
