...
<template>
  <div class="min-h-screen w-full flex items-center justify-center bg-[#f8fafc] relative overflow-hidden font-sans px-4 py-8 sm:px-6">
    ...
    <div class="relative z-10 w-full max-w-[520px] animate-fade-in-up sm:max-w-[560px]">
      <!-- Glass Card Container -->
      <div class="bg-white/70 backdrop-blur-xl border border-white/40 shadow-[0_20px_50px_rgba(0,0,0,0.05)] rounded-[32px] overflow-hidden p-7 sm:p-8 md:p-10 lg:p-11">
        
        <!-- Header -->
        <div class="text-center mb-9 sm:mb-10">
          <div class="inline-flex items-center justify-center w-20 h-20 rounded-[24px] bg-linear-to-tr from-indigo-600 to-blue-500 text-white mb-6 shadow-xl shadow-indigo-200/50 transform hover:scale-105 transition-transform duration-300 sm:w-22 sm:h-22">
            <Fingerprint class="w-10 h-10" />
          </div>
          <h2 class="text-4xl font-black text-slate-900 tracking-tight mb-2 sm:text-[2.8rem]">Đăng nhập</h2>
          <p class="mx-auto max-w-md text-slate-500 font-medium sm:text-lg">Chào mừng trở lại <span class="text-indigo-600 font-bold">HRM System</span></p>
        </div>
        ...
        <form @submit.prevent="handleLogin" class="space-y-6 sm:space-y-7">
          ...
          <div class="text-center mt-10">
            <p class="text-sm font-medium text-slate-400">
              ...
              <button type="button" class="ml-1 text-indigo-600 font-bold hover:underline" @click="showSupportModal = true">
                Hỗ trợ ngay
              </button>
            </p>
          </div>
    ...
    <Teleport to="body">
      <Transition name="fade-shake">
        <div v-if="showForgotModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative z-10 w-full max-w-md rounded-[28px] border border-white/50 bg-white p-6 shadow-[0_25px_80px_rgba(15,23,42,0.18)]">
            ...
            <div class="mt-6 flex gap-3">
              ...
              <BaseButton type="button" variant="primary" class="flex-1" :loading="forgotLoading" @click="handleForgotPassword">
                Gửi yêu cầu
              </BaseButton>
            </div>
          </div>
    ...
    <Teleport to="body">
      <Transition name="fade-shake">
        <div v-if="showSupportModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative z-10 w-full max-w-lg rounded-[30px] border border-white/50 bg-white p-6 shadow-[0_25px_80px_rgba(15,23,42,0.18)] md:p-7">
            <div class="flex items-start justify-between gap-4">
              <div>
                ...
                <p class="mt-2 text-sm font-medium leading-relaxed text-slate-500">
                  Nếu bạn không vào được hệ thống, hãy thử khôi phục mật khẩu hoặc liên hệ bộ phận hỗ trợ theo thông tin dưới đây.
                </p>
              </div>
              <button
                type="button"
